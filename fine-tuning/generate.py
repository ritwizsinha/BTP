...
def allocate_graph_tensors(n_total, batch_size):
    ...
    generated_nodes = torch.zeros((n_allocate, *node_shape[1:]), dtype=torch.float32, device="cpu")
    generated_edges = torch.zeros((n_allocate, *edge_shape[1:]), dtype=torch.float32, device="cpu")
    generated_n_nodes = torch.zeros(n_allocate, dtype=torch.int8, device="cpu")
    generated_agent_ll = torch.zeros(n_allocate, device="cpu")
    generated_prior_ll = torch.zeros(n_allocate, device="cpu")
    properly_terminated = torch.zeros(n_allocate, device="cpu")
    agent_loglike = torch.zeros(batch_size + 1, device="cpu")
    prior_loglike = torch.zeros(batch_size + 1, device="cpu")
...
def initialize_graph_batch(batch_size):
    ...
    # initialize tensors
    nodes = torch.zeros(node_shape, dtype=torch.float32, device="cpu")
    edges = torch.zeros(edge_shape, dtype=torch.float32, device="cpu")
    n_nodes = torch.zeros(batch_size + 1, dtype=torch.int64, device="cpu")

    # add a dummy non-empty graph at top, since models cannot receive as input
    # purely empty graphs
    nodes[0] = torch.ones(([1] + C.dim_nodes), device="cpu")
    edges[0, 0, 0, 0] = 1
    n_nodes[0] = 1
...
def reset_graphs(n_samples, idx, nodes, edges, n_nodes, agent_loglike, prior_loglike, batch_size):
    ...
    nodes_reset = torch.zeros(node_shape, dtype=torch.float32, device="cpu")
    edges_reset = torch.zeros(edge_shape, dtype=torch.float32, device="cpu")
    n_nodes_reset = torch.zeros(batch_size + 1, dtype=torch.int64, device="cpu")
    agent_loglike_reset = torch.zeros(batch_size + 1, dtype=torch.float32, device="cpu")
    prior_loglike_reset = torch.zeros(batch_size + 1, dtype=torch.float32, device="cpu")

    # reset the "bad" graphs with zero tensors
    if len(idx) > 0:
        nodes[idx] = torch.zeros(
            (len(idx), *node_shape[1:]), dtype=torch.float32, device="cpu"
        )
        edges[idx] = torch.zeros(
            (len(idx), *edge_shape[1:]), dtype=torch.float32, device="cpu"
        )
        n_nodes[idx] = torch.zeros(
            len(idx), dtype=torch.int64, device="cpu"
        )
        agent_loglike[idx] = torch.zeros(
            len(idx), dtype=torch.float32, device="cpu"
        )
    ...
    nodes_reset[0] = torch.ones(([1] + C.dim_nodes), device="cpu")
...
def graph_to_mol(node_features, edge_features, n_nodes):
    ...
    edge_mask = torch.triu(
        torch.ones((n_max_nodes, n_max_nodes), device="cpu"), diagonal=1
    )
