...
def allocate_graph_tensors(n_total, batch_size):
    ...
    nodes = torch.zeros((n_allocate, *node_shape[1:]), dtype=torch.float32, device="cpu")
    edges = torch.zeros((n_allocate, *edge_shape[1:]), dtype=torch.float32, device="cpu")
    n_nodes = torch.zeros(n_allocate, dtype=torch.int8, device="cpu")
    nlls = torch.zeros((batch_size + 1, nlls_shape), device="cpu")
    generated_nlls = torch.zeros((n_allocate, nlls_shape), device="cpu")
...
def reset_graphs(n_samples, idx, nodes, edges, n_nodes, nlls, batch_size):
    ...
    nlls_reset = torch.zeros(nlls_shape, dtype=torch.float32, device="cpu")
    ...
    if len(idx) > 0:
        ...
        nlls[idx] = torch.zeros(
            (len(idx), *nlls_shape[1:]), dtype=torch.float32, device="cpu"
        )
