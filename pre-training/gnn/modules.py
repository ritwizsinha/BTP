...
class GlobalReadout(torch.nn.Module):
    ...
    def forward(self, node_level_output, graph_embedding_batch):
        """ Defines forward pass.
        """
        self.fAddNet1 = self.fAddNet1.to("cpu", non_blocking=True)
        self.fConnNet1 = self.fConnNet1.to("cpu", non_blocking=True)
        self.fAddNet2 = self.fAddNet2.to("cpu", non_blocking=True)
        self.fConnNet2 = self.fConnNet2.to("cpu", non_blocking=True)
        self.fTermNet2 = self.fTermNet2.to("cpu", non_blocking=True)

        # get preliminary f_add and f_conn
        f_add_1 = self.fAddNet1(node_level_output)
        f_conn_1 = self.fConnNet1(node_level_output)

        f_add_1 = f_add_1.to("cpu", non_blocking=True)
        f_conn_1 = f_conn_1.to("cpu", non_blocking=True)
        ...
        f_add_2 = f_add_2.to("cpu", non_blocking=True)
        f_conn_2 = f_conn_2.to("cpu", non_blocking=True)
        f_term_2 = f_term_2.to("cpu", non_blocking=True)
